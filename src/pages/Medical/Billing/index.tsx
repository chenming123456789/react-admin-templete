...
const Billing: React.FC = () => {
  ...
  const handleAddItem = () => {
    const randomItem = itemLibrary[Math.floor(Math.random() * itemLibrary.length)];
  ...
  const handleSettle = () => {
    ...
    message.success(`结算成功！总计金额: ￥${total.toFixed(2)}，支付方式: ${paymentMethod}`);
    setItems([]);
  };
  ...
  const columns = [
    ...
    { 
      ...
      render: (_: any, r: BillingItem) => (
        <InputNumber min={1} value={r.quantity} onChange={(v) => updateQuantity(r.id, v || 1)} />
      )
    },
    { 
      title: '小计', 
      key: 'total', 
      render: (_: any, r: BillingItem) => `￥${(r.price * r.quantity).toFixed(2)}` 
    },
    {
      title: '操作',
      key: 'action',
      render: (_: any, record: BillingItem) => (
        <Button type="link" danger onClick={() => removeItem(record.id)}>移除</Button>
      ),
    },
  ];
          ...
          >
            <div className={styles.searchSection}>
              <Space>
                ...
                />
                <Button onClick={() => message.info('正在查询患者信息...')}>查询</Button>
                {patientSearch && <Text type="success">匹配到患者: 张三 (男, 35岁)</Text>}
              </Space>
            </div>
            <Table columns={columns} dataSource={items} rowKey="id" pagination={false} size="middle" />
          </Card>
        </Col>
        ...
        <Col span={8}>
          <Card title="结算中心" bordered={false}>
            <div className={styles.settlementSection}>
              ...
              <Button type="primary" size="large" block icon={<CreditCardOutlined />} onClick={handleSettle} disabled={items.length === 0}>
