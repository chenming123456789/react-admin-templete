...
const RoleManagement: React.FC = () => {
  ...
  const fetchList = () => {
    ...
    setTimeout(() => {
      setData([
        { id: '1', name: '超级管理员', code: 'ROLE_ADMIN', description: '系统最高权限', createTime: '2024-01-01' },
        { id: '2', name: '运营人员', code: 'ROLE_OPERATOR', description: '内容维护权限', createTime: '2024-02-01' },
      ]);
      setLoading(false);
    }, 500);
  };
  ...
  const columns = [
    ...
    { title: '创建时间', dataIndex: 'createTime', key: 'createTime' },
    {
      ...
      render: (_: any, record: RoleItem) => (
        <Space size="middle">
          <Button type="link" icon={<SafetyCertificateOutlined />}>分配权限</Button>
          <Button type="link" icon={<EditOutlined />} onClick={() => {
            setEditingItem(record);
            form.setFieldsValue(record);
            setIsModalOpen(true);
          }}>编辑</Button>
          <Popconfirm title="确定删除吗？" onConfirm={() => message.success('删除成功')}>
